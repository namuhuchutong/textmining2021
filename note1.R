...
# 빈도수와 tf-idf에서 상위 20개 중, 성차별 명사 추출
...
word_new_comments1_s <- word_new_comments1 %>% group_by(id, value) %>% summarise(score = sum(polarity)) %>% ungroup()
word_new_comments2_s <- word_new_comments2 %>% group_by(id, value) %>% summarise(score = sum(polarity)) %>% ungroup()
...
unlabeled_data_pos <- unlabeled_data %>% unnest_tokens(input = value, output = word, token = SimplePos22, drop = F)
...
word_cors <- unlabeled_comments %>% add_count(word) %>% filter(n >= 70) %>% pairwise_cor(item = word, feature = id, sort = T)
graph_cors <- word_cors %>% filter(correlation >= 0.8) %>% as_tbl_graph(directed = F) %>% mutate(centrality = centrality_degree(), group = as.factor(group_infomap()))
